...
# Function ----
RFX_fx<-function(outname,AYR,endyr,datadir,outdir,admboutdir,regional=T){ #note: outname needs to match the RACE biomass file
  # Data Prep ----
  RFX_data<-read.csv(paste(datadir,"/RACE_Biomass_",outname,".csv",sep=""),header=T)
  # Get rid of NBS survey data, causes problems and mostly zeros
  RFX_data <- RFX_data %>% 
    filter(SURVEY != "NBS")
  ...
  for (i in 1:nrow(unqkey)){
    ...
    #there are no sub regions for either EBS/NBS survey in this code, so loopdat is the same as ld2
    if(str_detect(unqkey[i,1], "^(EBS_)")) ld2<-loopdat
    #if(unqkey[i,1]=="NBS") ld2<-loopdat
    ...
    ## RFX model info, versions model outputs for later reference
    survdir <- paste(admboutdir, "/", unqkey[i,1], sep = "")
    dir.create(survdir, showWarnings = T)
    specdir <- paste(survdir, "/", unqkey[i,2], sep = "")
    dir.create(specdir, showWarnings = T)
    mtime <- Sys.time()
    mtime <- gsub(" ", "_", mtime)
    mtime <- gsub("-", "_", mtime)
    mtime <- gsub(":", "_", mtime)
    redir <- paste(specdir, "/", unqkey[i,1], unqkey[i,2], mtime, sep = "")
    dir.create(redir, showWarnings = T)
    filestocopy <- c("re.dat", "re.par", "re.rep", "RE.TPL", "rwout.rep")
    for (j in 1: length(filestocopy)){
      file.copy(from = paste(codedir, "/", filestocopy[j], sep=""), to = redir, overwrite = T, copy.mode = T, copy.date = T)
    }
  }
