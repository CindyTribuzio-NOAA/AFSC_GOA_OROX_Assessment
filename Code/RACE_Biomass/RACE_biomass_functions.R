...
RACE_BIOMASS<-function(Species,outname,SYR,datadir,outdir){
  ...
  #RACE surveys
  SURVEYS<-read.csv(paste(datadir,"/RACE_SURVEY",SYR,".csv",sep=""),header=T)
  ...
  #get rid of EGOA data for DSR species
  DSR_codes <- c(30410, 30120, 30370, 30320, 30270, 30380, 30470)
  catch_DSR_GOA <- catch2 %>% 
    filter(SPECIES_CODE %in% DSR_codes,
           REGULATORY_AREA_NAME != "EASTERN GOA",
           SURVEY == "GOA")
  catch_DSR_BSAI <- catch2 %>% 
    filter(SPECIES_CODE %in% DSR_codes,
           SURVEY != "GOA")
  catch2 <- catch2 %>% 
    filter(SPECIES_CODE %nin% DSR_codes) %>% 
    bind_rows(catch_DSR_GOA, catch_DSR_BSAI)
